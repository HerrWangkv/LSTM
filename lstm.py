...
#character to index: char_to_ix['character']
char_to_ix = {ch: i for i, ch in enumerate(chars)}
#index to character: ix_to_char[index]
ix_to_char = {i: ch for i, ch in enumerate(chars)}
std = 0.1
#sys.argv stands for a list of the input parameter from command line. Note: sys.argv[0] should be the name of the file(e.g.'a.py')
option = sys.argv[1]
...
def forward(inputs, targets, memory):
    ...
    assert inputs.shape==(input_length,batch_size)
    ...
    for t in range(input_length):
        ...
        # convert word indices to word embeddinc_s
        wes[t] = np.dot(Wex, xs[t])
        assert wes[t].shape == (emb_size,batch_size)
        
        # LSTM cell operation
        # first concatenate the input and h to get z
        zs[t] = np.row_stack((hs[t - 1], wes[t]))
        assert zs[t].shape == (concat_size,batch_size)

        # compute the forget gate
        # f = sigmoid(Wf * z + bf)
        fs[t] = sigmoid(Wf@zs[t]+bf)
        assert fs[t].shape == (hidden_size,batch_size)
        
        # compute the input gate
        # i = sigmoid(Wi * z + bi)
        ins[t] = sigmoid(Wi@zs[t]+bi)
        assert ins[t].shape == (hidden_size,batch_size)
        
        # compute the candidate memory
        # c_ = tanh(Wc * z + bc)
        c_s[t] = np.tanh(Wc@zs[t]+bc)
        assert c_s[t].shape == (hidden_size,batch_size)

        # new memory: applying forget gate on the previous memory
        # and then adding the input gate on the candidate memory
        # gt = f * g(t-1) + i * g
        cs[t] = fs[t]*cs[t-1]+ins[t]*c_s[t]
        assert cs[t].shape == (hidden_size,batch_size)

        # output gate
        #o = sigmoid(Wo * z + bo)
        os[t] = sigmoid(Wo@zs[t]+bo)
        assert os[t].shape == (hidden_size,batch_size)
        ...
        assert hs[t].shape == (hidden_size,batch_size)
        
        ys[t] = Why@hs[t]+by
        assert ys[t].shape == (vocab_size,batch_size)
        
        ps[t] = softmax(ys[t])
        assert ps[t].shape == (vocab_size,batch_size)
...
def backward(activations, clipping=True):
    ...
    for t in reversed(range(input_length)):
        ...
        #d loss_t/d by = d loss_t/d ys[t]
        dl_dy = ps[t]-ls[t]
        assert dl_dy.shape == (vocab_size,batch_size)
        dl_db = np.sum(dl_dy,axis=1).reshape(-1,1)
        assert dl_db.shape == (vocab_size,1)
        dby = dby+dl_db
        
        #d loss_t/d Why
        dl_dw = (ps[t]-ls[t])@hs[t].T
        assert dl_dw.shape == (vocab_size, hidden_size)
        dWhy=dWhy+dl_dw
        
        #d loss_t/d hs[t]
        dl_dh = dhnext+Why.T@dl_dy
        assert dl_dh.shape == (hidden_size,batch_size)
        
        #d loss_t/d os[t]
        dl_do = dl_dh*np.tanh(cs[t])
        assert dl_do.shape == (hidden_size,batch_size)
        
        # d loss_t/d cs[t]
        dl_dc = dcnext+dl_dh*os[t]*dtanh(np.tanh(cs[t]))
        assert dl_dc.shape == (hidden_size,batch_size)
        
        #d loss_t/d bo
        dl_db = np.sum(dl_do*dsigmoid(os[t]),axis=1).reshape(-1,1)
        assert dl_db.shape == (hidden_size,1)
        dbo=dbo+dl_db
        
        #d loss_t/d Wo
        dl_dwz = dl_do*dsigmoid(os[t])
        assert dl_dwz.shape == (hidden_size,batch_size)
        dl_dw = dl_dwz@zs[t].T
        assert dl_dw.shape == (hidden_size, concat_size)
        dWo=dWo+dl_dw
        
        #d loss_t/d zs[t]1
        dl_dz = Wo.T@dl_dwz
        assert dl_dz.shape == (concat_size,batch_size)
        
        #d loss_t/d fs[t]
        dl_df = dl_dc*cs[t-1]
        assert dl_df.shape == (hidden_size,batch_size)
        
        #d loss_t/d ins[t]
        dl_di = dl_dc*c_s[t]
        assert dl_di.shape == (hidden_size,batch_size)
        
        #d loss_t/d c_s[t]
        dl_dc_ = dl_dc*ins[t]
        assert dl_dc_.shape == (hidden_size,batch_size)
        
        #d loss_t/d bf
        dl_db = np.sum(dl_df*dsigmoid(fs[t]),axis=1).reshape(-1,1)
        assert dl_db.shape == (hidden_size,1)
        dbf=dbf+dl_db
        
        #d loss_t/d Wf
        dl_dwz = dl_df*dsigmoid(fs[t])
        dl_dw = dl_dwz@zs[t].T
        assert dl_dw.shape == (hidden_size, concat_size)
        dWf=dWf+dl_dw
        
        #d loss_t/d zs[t]2
        dl_dz = dl_dz+Wf.T@dl_dwz
        assert dl_dz.shape == (concat_size,batch_size)
        
        #d loss_t/d bi
        dl_db = np.sum(dl_di*dsigmoid(ins[t]),axis=1).reshape(-1,1)
        assert dl_db.shape == (hidden_size,1)
        dbi=dbi+dl_db
        
        #d loss_t/d Wi
        dl_dwz = dl_di*dsigmoid(ins[t])
        dl_dw = dl_dwz@zs[t].T
        assert dl_dw.shape == (hidden_size, concat_size)
        dWi=dWi+dl_dw
        
        #d loss_t/d zs[t]3
        dl_dz = dl_dz+Wi.T@dl_dwz
        assert dl_dz.shape == (concat_size,batch_size)
        
        #d loss_t/d bc
        dl_db = np.sum(dl_dc_*dtanh(c_s[t]),axis=1).reshape(-1,1)
        assert dl_db.shape == (hidden_size,1)
        dbc=dbc+dl_db
        
        #d loss_t/d Wc
        dl_dwz = dl_dc_*dtanh(c_s[t])
        dl_dw = dl_dwz@zs[t].T
        assert dl_dw.shape == (hidden_size, concat_size)
        dWc=dWc+dl_dw
        
        #d loss_t/d zs[t]4
        dl_dz = dl_dz+Wc.T@dl_dwz
        assert dl_dz.shape == (concat_size,batch_size)
        
        #d loss_t/d Wex[t]
        dl_de = dl_dz[hidden_size:]
        dl_dw = dl_de@xs[t].T
        assert dl_dw.shape == (emb_size, vocab_size)
        dWex=dWex+dl_dw
        
        #dhnext
        dhnext = dl_dz[:hidden_size]
...
if option == 'train':
    ...
    while True:
        ...
        if n_updates >= max_updates:
            ...
            fh.write('***** After training %d iterations, the generated text is as follows *****\n\n'%n_updates)
            fh.write(txt)
            fh.close()
            break
